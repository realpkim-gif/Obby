...
while running:
    ...
    # ---------- DEBUG: WHICH SOLID + WHERE ----------
    """
    
    for idx, solid in enumerate(solids):
        # probes around Mario (1 px outside his rect)
        foot = pygame.Rect(mario_rect.x + 2, mario_rect.bottom, mario_rect.w - 4, 2)
        head = pygame.Rect(mario_rect.x + 2, mario_rect.top - 2, mario_rect.w - 4, 2)
        left = pygame.Rect(mario_rect.left - 2, mario_rect.y + 2, 2, mario_rect.h - 4)
        right = pygame.Rect(mario_rect.right, mario_rect.y + 2, 2, mario_rect.h - 4)

        if foot.colliderect(solid):
            print(f"Touching solids[{idx}] at TOP (standing on it)")

        if head.colliderect(solid):
            print(f"Touching solids[{idx}] at BOTTOM (hit head)")

        if left.colliderect(solid):
            print(f"Touching solids[{idx}] at RIGHT side (wall)")

        if right.colliderect(solid):
            print(f"Touching solids[{idx}] at LEFT side (wall)")
...
#---------- DEATH PIT / FALL DEATH ----------
    ...
    # Grounds (draw all)
    pygame.draw.rect(screen, GREEN, (ground_rect.x  - camera_x, ground_rect.y,  ground_rect.width,  ground_rect.height))
    pygame.draw.rect(screen, GREEN, (ground_rect2.x - camera_x, ground_rect2.y, ground_rect2.width, ground_rect2.height))
    pygame.draw.rect(screen, GREEN, (ground_rect3.x - camera_x, ground_rect3.y, ground_rect3.width, ground_rect3.height))
    pygame.draw.rect(screen, GREEN, (ground_rect4.x - camera_x, ground_rect4.y, ground_rect4.width, ground_rect4.height))
    pygame.draw.rect(screen, GREEN, (ground_rect5.x - camera_x, ground_rect5.y, ground_rect5.width, ground_rect5.height))
    pygame.draw.rect(screen, GREEN, (ground_rect6.x - camera_x, ground_rect6.y, ground_rect6.width, ground_rect6.height))
    pygame.draw.rect(screen, GREEN, (ground_rect7.x - camera_x, ground_rect7.y, ground_rect7.width, ground_rect7.height))
    pygame.draw.rect(screen, GREEN, (ground_rect8.x - camera_x, ground_rect8.y, ground_rect8.width, ground_rect8.height))
    pygame.draw.rect(screen, GREEN, (ground_rect9.x - camera_x, ground_rect9.y, ground_rect9.width, ground_rect9.height))
    pygame.draw.rect(screen, GREEN, (ground_rect10.x - camera_x, ground_rect10.y, ground_rect10.width, ground_rect10.height))
    pygame.draw.rect(screen, GREEN, (ground_rect11.x - camera_x, ground_rect11.y, ground_rect11.width, ground_rect11.height))
